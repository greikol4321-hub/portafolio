...
export default function Home() {
  return <div className="wander-page">
    ...
    <main id="inicio">
      <section className="portfolio-hero"><Reveal><p className="eyebrow">PORTAFOLIO / DESARROLLO WEB / 2026</p><h1><TextReveal text="Software para" /><br /><em><TextReveal text="problemas reales." delay={0.12} /></em></h1><p className="lead">Soy Greikol Quesada. Desarrollo sistemas para colegios, negocios y turismo: entradas, paneles y herramientas fáciles de usar.</p><div className="hero-links"><a className="hero-primary" href="#trabajo">Ver proyectos</a><a href="https://github.com/greikol4321-hub" target="_blank" rel="noreferrer">GitHub ↗</a></div></Reveal><Reveal delay={0.15} direction="left" className="proof-board">
...
<p className="proof-note">para gente que necesita que las cosas funcionen.</p>
      ...
      <section className="stack-section"><Reveal><p className="eyebrow">HERRAMIENTAS</p><h2><TextReveal text="Con esto trabajo." /></h2></Reveal><StaggerReveal className="stack-grid">{stackGroups.map(([title, ...items], index) => <StaggerItem key={title} className="stack-group"><h3>{title}</h3><div>{items.map((item) => <span key={item}>{item}</span>)}</div></StaggerItem>)}</StaggerReveal></section>
      <section className="about-section" id="perfil"><Reveal><p className="eyebrow">PERFIL</p><h2><TextReveal text="Menos fricción." /><br /><em><TextReveal text="Mejores resultados." delay={0.08} /></em></h2></Reveal><Reveal delay={0.08} className="about-copy"><p>Soy Greikol Yanfred Quesada Amador, estudiante de duodécimo año en Desarrollo Web en el Colegio Técnico Profesional de Quepos.
